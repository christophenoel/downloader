...
public interface IProductDownloadListener {
     ...
     *            provide further details for error cases.
     * @param exception
     *            An optional parameter which provides the exception thrown by the 
     *            plugin if an error has ocurred. This allows the Download Manager
     *            to provide the user with further details of the error and allows
     *            granular configuration of email notifications based on the error
     *            type.
     *            <p>The exception is of type  {@link int_.esa.eo.ngeo.downloadmanager.exception.DMPluginException}
     *            and can be one of the following sub types to provide more detail:</p>
     *            <ul>
     *              <li>{@link int_.esa.eo.ngeo.downloadmanager.exception.AuthenticationException} - Unable to authenticate with UM-SSO</li>
     *              <li>{@link int_.esa.eo.ngeo.downloadmanager.exception.FileSystemWriteException} - e.g. disk full, a write lock on file to be written to.</li>
     *              <li>{@link int_.esa.eo.ngeo.downloadmanager.exception.ProductUnavailableException} - The plugin has been unable to download the product after a number of retries.</li>
     *              <li>{@link int_.esa.eo.ngeo.downloadmanager.exception.UnexpectedResponseException} - for example an HTTP response code which is not expected from a Product Facility.</li>
     *            </ul>
     */
    void progress(Integer progressPercentage, Long downloadedSize,
